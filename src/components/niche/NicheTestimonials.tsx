      ...
      <div className="container">
        ...
        <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 md:grid md:grid-cols-3 md:overflow-visible md:pb-0 md:gap-6">
          {testimonials.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex-shrink-0 w-[300px] md:w-auto bg-card rounded-2xl p-6 md:p-8 border border-border/40 hover:shadow-lg transition-all duration-300 relative">
